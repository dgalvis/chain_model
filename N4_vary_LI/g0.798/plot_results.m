...
plot_curve('run_loop_B', colors(5,:), '--');
...
scatter(LI(end), x(end), 50, 'filled', 'd', 'MarkerEdgeColor', colors(8,:), 'MarkerFaceColor',colors(8,:));

[LI, ~, ~,  x] = find_special_points('run_loop_B', 'SN');
scatter(LI(1), x(1), 50, 'filled', 'd', 'MarkerEdgeColor', colors(8,:), 'MarkerFaceColor',colors(8,:));
scatter(LI(end), x(end), 50, 'filled', 'd', 'MarkerEdgeColor', colors(8,:), 'MarkerFaceColor',colors(8,:));
...
exportgraphics(gcf, '../g0.798.eps', 'Resolution', 300);
